...
specific error conditions in your code. They are derived from the built-in Exception class
or any of its sub classes.

User-defined exceptions provide more precise control over error handling in your
application −
    ...
     Maintainability − They centralize error handling logic, making your code easier
        to maintain.'''
...
-----------------
Explanation
     Inheritance − By inheriting from "Exception", your custom exception will have
        the same behavior and attributes as the built-in exceptions.
     Class Definition − The class is defined using the standard Python class syntax.
        For simple custom exceptions, you can define an empty class body using the "pass"
        statement.'''
...
---------------------
Explanation
     Attributes − Define attributes such as "age" and "message" to store information
        about the error.
     Initialization − The "__init__" method initializes these attributes. The
        "super().__init__(self.message)" call ensures that the base "Exception" class is
        properly initialized with the error message.
     Default Message − A default message is provided, but you can override it when
...
----------------------
Explanation
     __str__ Method − The "__str__" method returns a string representation of the
        exception. This is what will be displayed when the exception is printed.
     Custom Message − Customize the message to include relevant information, such
        as the provided age in this example.'''

''' Raising User-Defined Exceptions
Once you have defined a custom exception, you can raise it in your code to signify specific
error conditions. Raising user-defined exceptions involves using the raise statement, which
can be done with or without custom messages and attributes.
...
and respond to the specific conditions that your custom exceptions represent. This allows
your program to handle errors gracefully and continue running or to take specific actions
